// MOUNA – Speech Recognition using browser Web Speech API + Backend Whisper Integration
// Captures all audio from GMeet (including other speakers) and sends to MOUNA Whisper backend
// Displays signs for all speakers in the call

const WHISPER_SERVER = 'http://localhost:8000'; // MOUNA Whisper service endpoint
...
class WhisperTranscriber {
  ...
  async checkConnection() {
    try {
      ...
      console.log('⚠️ Whisper server unavailable:', e.message);
      this.isConnected = false;
    }
  }
...
// Sign video map - ALL AVAILABLE VIDEOS FROM MOUNA
const signVideoMap = {
...
let videoQueue = []; // each item: {url, label}
let isPlaying = false;
let currentVideoIndex = 1; // Track which video element is currently active
...
// Function to spell out words letter by letter
...
// update the overlay text shown beneath the video
function updateOverlayText(text, fullWord = null, currentLetterIndex = -1) {
  ...
  if (fullWord && currentLetterIndex >= 0) {
    // Show full word with current letter underlined
...
function playNextVideo() {
  ...
  // Determine which video to use for the new content
  ...
  nextVideo.oncanplay = () => {
    ...
    nextVideo.play().then(() => {
      console.log('▶️ Video started playing (crossfade)');
      ...
      // If autoplay failed due to policy, try to play on user interaction
      if (err.name === 'NotAllowedError') {
        ...
        const playOnClick = () => {
          ...
          nextVideo.play().then(() => {
            console.log('▶️ Video started after user interaction');
            nextVideo.onended = () => {
              nextVideo.onended = null;
              playNextVideo();
            };
...
// Draggable and zoomable overlay functionality
...
function makeOverlayDraggable(overlay) {
  ...
  overlay.addEventListener('mousedown', (e) => {
    // Don't start drag if clicking on zoom buttons
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    // Keep overlay within viewport bounds (accounting for scale)
...
window.addEventListener('load', () => {
  ...
  // Attempt to capture GMeet audio
  captureGMeetAudio();

  // Create overlay immediately so it's always visible
  getOverlay();
});

// Audio capture functions for GMeet integration
async function captureGMeetAudio() {
  ...
  try {
    ...
    processor.onaudioprocess = (e) => {
      ...
      // Process chunks every ~2 seconds or when we have enough data
      const now = Date.now();
...
async function processAudioChunks(chunks) {
  ...
  try {
    ...
    if (transcription && transcription.trim()) {
      ...
      words.forEach(word => {
        if (word.trim()) {
          // Trigger the same word detection logic
